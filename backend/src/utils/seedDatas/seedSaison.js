...
async function seedSaisons() {
  try {
    ...
    const saisons = [
      {
        ...
        description:
          'La première saison où les héros se rencontrent et commencent leur quête.',
        jeu: jeux[0]._id,
        dateDebut: new Date(),
        statut: 'en_cours',
      },
      {
        ...
        description:
          'Les héros font face à de nouveaux défis et des ennemis plus puissants.',
        jeu: jeux[0]._id,
      ...
      },
      {
        ...
        titre: 'Les Origines',
        description:
          'Découvrez les origines des personnages dans ce nouveau jeu passionnant.',
    ...
    for (const saison of saisons) {
      const existingSaison = await Saison.findOne({
      ...
      if (!existingSaison) {
        ...
        console.log(
          `Saison créée : ${nouvelleSaison.titre} (Jeu: ${nouvelleSaison.jeu})`
        );

        // Mettre à jour le jeu associé avec l'ObjectId de la nouvelle saison
        await Jeu.findByIdAndUpdate(saison.jeu, {
          $push: { saisons: nouvelleSaison._id },
        });
      } else {
        console.log(
          `La saison ${saison.numero} pour le jeu ${saison.jeu} existe déjà.`
        );
      }
    }
